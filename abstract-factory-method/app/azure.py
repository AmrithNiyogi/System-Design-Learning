...
class AzureSignedUrlGenerator(SignedUrlGenerator):

    def generate_upload_url(self, upload_request: dict) -> str:
        filename = upload_request["filename"]
        return f"https://azure.com/docs/{filename}?upload=1"

    def generate_download_url(self, document_id: str) -> str:
        return f"https://azure.com/docs/{document_id}?download=1"


class CosmosMetadataRepository(MetadataRepository):
